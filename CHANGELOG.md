...
### Changed
- `objects::message::Message`'s `peer_id` field is now an `Option<Integer>` (was `Integer`), so that that object can be used in `message_typing_state` Callback API event, and also just in case.
